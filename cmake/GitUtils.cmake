...
function(git_clone)
    ...
    if(EXISTS ${${SOURCE_DIR}})
        ...
        if(git_result EQUAL "0")
                ...
                if(NOT git_result EQUAL "0")
                    ...
                    if(NOT PARGS_QUIET)
                        message(FATAL_ERROR "${PARGS_PROJECT_NAME}  submodule update error")
                    endif()
                    return()
                endif()
        else()
            set(${CLONE_RESULT} FALSE CACHE INTERNAL "" FORCE)
            if(NOT PARGS_QUIET)
                message(FATAL_ERROR "${PARGS_PROJECT_NAME} pull error")
            endif()
            return()
        endif()
    else()
        ...
        if(NOT git_result EQUAL "0")
            set(${CLONE_RESULT} FALSE CACHE INTERNAL "" FORCE)
            if(NOT PARGS_QUIET)
                message(FATAL_ERROR "${PARGS_PROJECT_NAME} clone error")
            endif()
            return()
        endif()        
    endif()
    ...
    if(NOT git_result EQUAL "0")
        set(${CLONE_RESULT} FALSE CACHE INTERNAL "" FORCE)
        if(NOT PARGS_QUIET)
            message(FATAL_ERROR "${PARGS_PROJECT_NAME} some error happens. ${git_output}")
        endif()
        return()
    else()
